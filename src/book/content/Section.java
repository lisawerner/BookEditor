...
public class Section extends SerializedObject {
	...
	public List<Person> getPersonByTag(){
		return getTagsByTagType("book.person.Person").stream().map(tag -> Book.getInstance().getSociety().getPerson(tag.getRefID())).collect(Collectors.toList());
	}
	
	public List<Place> getPlaceByTag(){
		return getTagsByTagType("book.world.Place").stream().map(tag -> Book.getInstance().getWorld().getPlace(tag.getRefID())).collect(Collectors.toList());
	}
